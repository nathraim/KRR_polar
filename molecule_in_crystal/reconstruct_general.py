...
""" This scripts reads an MD trajectory for paracetamol, and reconstructs the molecules as a whole.
...
def translate(line,shift):
   ...
   #   pos = np.array(list(map(float,line[j].split()[1:4])))
   #   atmtyp = line[j].split()[0]
   #   pos = pos + shift
   #   line[j] = '       '+atmtyp+'  '+str("{0:.5e}".format(pos[0]))+'  '+str("{0:.5e}".format(pos[1]))+'  '+str("{0:.5e}".format(pos[2]))+'\n'
...
while (counter < nconf):
  ...
  for i in range(2,nat+2,1): # Go through all the atomic positions
    ...
    diff = pos-posref[i-2] # Difference between the reference structure and the current one (1 atom)

    # The following assumes that during MD, the positions are all relatively close to each other, in the sense that the amplitude of the atoms is much less than on of the lattice vectors
    if (abs(diff[0]) >3 or abs(diff[1]) >3 or abs(diff[2]) >3): # Check if the current position is too different from the reference
      for disp in comb: # Go through all the possible combinations of unit-cell parameters
        ...
        if (abs(diff2[0]) <3 and abs(diff2[1]) <3 and abs(diff2[2]) <3):
          data[k+i] = translate(data[k+i],disp)
          #print (k/82+1,i, pos, data[k+i])
          break
